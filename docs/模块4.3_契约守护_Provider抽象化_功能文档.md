...
专门针对DeepSeek R1的推理能力优化，能够捕获AI的思考过程（reasoning content）。

#### 核心功能

1. **推理链监控**: 捕获DeepSeek R1的思考过程（Reasoning content）
2. **OpenAI协议对齐**: 通过标准的OpenAI兼容接口实现快速接入
3. **认知风险分析**: 分析AI在得出结论前如何"联想"到竞品

#### 推理链提取

系统能够从DeepSeek R1响应中提取推理内容：
- 直接从`reasoning`字段提取
- 从`reasoning_content`字段提取
- 从`thinking_process`字段提取
- 通过模式匹配从内容中分离推理和最终答案
