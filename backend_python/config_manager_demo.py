...
class Config:
    ...
    def _load_from_env(self):
        """从环境变量加载配置"""
        # 支持的平台列表
        supported_platforms = ['openai', 'qwen', 'anthropic', 'google', 'chatgpt']
...
if __name__ == "__main__":
    ...
    os.environ['QWEN_API_KEY'] = 'your-qwen-key'
    
    # 创建配置实例
    config = Config()
    
    # 获取特定平台的配置
    openai_config = config.get_platform_config('openai')
    if openai_config:
        print(f"OpenAI 配置: Base URL={openai_config.base_url}, Temperature={openai_config.default_temperature}")
    
    # 检查API密钥完整性
    missing_keys = config.check_missing_keys()
    if missing_keys:
        print(f"缺少API密钥的平台: {missing_keys}")
    else:
        print("所有平台的API密钥均已配置")
    ...
    config.add_platform('custom', custom_config)
    
    print(f"更新后的可用平台: {config.get_available_platforms()}")