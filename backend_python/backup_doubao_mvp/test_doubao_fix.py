...
def test_doubao_adapter():
    ...
    try:
        ...
        # 测试使用特定部署ID的豆包适配器
        print("1. 创建豆包适配器 (使用部署ID ep-20260212000000-gd5tq)...")
        ...
        print("2. 发送测试请求...")
        # 发送测试请求
        response = adapter.send_prompt("你好，请简单介绍一下自己。", execution_id="test-execution-001")
...
def test_doubao_adapter_with_different_model():
    """测试使用不同模型名称的豆包适配器"""
    print("\n=== 测试不同模型名称格式的豆包适配器 ===")
    ...
    for model_name in model_names:
        ...
        try:
            ...
            response = adapter.send_prompt("你好。", execution_id=f"test-{model_name}")
...
def main():
    ...
    # 测试主要API
    results.append(("主要测试", test_doubao_adapter()))
    
    # 测试不同模型名称格式
    results.append(("不同格式测试", test_doubao_adapter_with_different_model()))
