#!/usr/bin/env python3
"""
AI 响应日志记录模块 V2 - 增强版（线程安全）
用于保存 AI 搜索平台的完整反馈结果，支持模型训练、性能分析和问题排查

线程安全特性：
- 使用 threading.Lock() 保护文件写入操作
- 防止 NxM 并发写入时导致 JSONL 格式损坏
"""
