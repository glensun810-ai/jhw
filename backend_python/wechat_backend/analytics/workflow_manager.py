...
class WebhookManager:
    ...
    def send_webhook(self, webhook_url: str, payload: Dict[str, Any], task_id: str, max_retries: int = 3) -> bool:
        ...
        Returns:
            bool: 是否发送成功
        """
        debug_log("WEBHOOK_DISPATCH", task_id, task_id, f"Starting webhook dispatch for task {task_id} to {webhook_url}")
        ...
        while retry_count <= max_retries:
            try:
                ...
                if success:
                    debug_log("WEBHOOK_DISPATCH", task_id, f"Webhook dispatch successful for task {task_id}")
                    return True
                else:
                    # 如果失败，检查是否需要重试
                    if retry_count < max_retries:
                        ...
                        debug_log("WEBHOOK_DISPATCH", task_id, f"Webhook failed for task {task_id}, retrying in {delay_seconds}s (attempt {retry_count + 1}/{max_retries})")
                        time.sleep(delay_seconds)
                        retry_count += 1
                    else:
                        debug_log("WEBHOOK_DISPATCH", task_id, f"Webhook failed for task {task_id} after {max_retries} retries")
                        return False

            except CircuitBreakerOpenError as e:
                exception_log(task_id, "CIRCUIT_BREAKER_ERROR", f"Circuit breaker is open for task {task_id}, skipping webhook to {webhook_url}. Error: {str(e)}")
                return False
            except Exception as e:
                exception_log(f"Error sending webhook for task {task_id}: {str(e)}")
                if retry_count < max_retries:
                    delay_seconds = min(60 * (2 ** retry_count), 300)
                    debug_log("WEBHOOK_DISPATCH", task_id, f"Retrying webhook for task {task_id} in {delay_seconds}s (attempt {retry_count + 1}/{max_retries})")
                    time.sleep(delay_seconds)
                    retry_count += 1
                else:
                    return False
    ...
    def _send_webhook_internal(self, webhook_url: str, payload: Dict[str, Any], task_id: str) -> bool:
        ...
        try:
            ...
            exception_log(task_id, "TIMEOUT_ERROR", f"Webhook request timed out for {webhook_url} (task {task_id})")
            # 超时应该传播给断路器以触发熔断
            raise requests.exceptions.Timeout(f"Webhook request to {webhook_url} timed out")
        except requests.exceptions.RequestException as e:
            exception_log(task_id, "REQUEST_ERROR", f"Webhook request failed for {webhook_url} (task {task_id}): {str(e)}")
            # 其他请求异常也应传播给断路器
            raise e
        except Exception as e:
            exception_log(task_id, "UNEXPECTED_ERROR", f"Unexpected error sending webhook to {webhook_url} (task {task_id}): {str(e)}")
            return False
        ...
        def worker():
            while True:
                try:
                    ...
                    exception_log("BACKGROUND_PROCESSOR", "PROCESSOR_ERROR", f"Error in background processor: {str(e)}")

        # 启动后台线程
        debug_log("WORKFLOW_MANAGER", "SYSTEM", "Starting background processor thread")
        ...
        def retry_worker():
            while True:
                try:
                    ...
                    exception_log("RETRY_PROCESSOR", "PROCESSOR_ERROR", f"Error in retry processor: {str(e)}")
                    time.sleep(1)

        # 启动重试处理线程
        debug_log("WORKFLOW_MANAGER", "SYSTEM", "Starting retry processor thread")
