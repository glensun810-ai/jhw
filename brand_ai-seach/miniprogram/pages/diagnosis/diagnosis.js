...
Page({
  ...
  async handleComplete(result) {
    ...
    try {
      // 【P0 关键修复 - 2026-03-16】使用 reportService 获取完整报告数据
      // reportService.getFullReport() 返回的是经过处理的完整报告数据
      // 包含 brandDistribution, sentimentDistribution, keywords 等
      console.log('[DiagnosisPage] 从 reportService 获取完整报告数据...');
      
      let fullReport = null;
      try {
        // 尝试 1: 使用 reportService（返回已处理的数据）
        fullReport = await reportService.getFullReport(this.data.executionId);
        console.log('[DiagnosisPage] ✅ reportService 返回数据');
      } catch (reportErr) {
        console.warn('[DiagnosisPage] ⚠️ reportService 失败，降级到 diagnosisService:', reportErr);
        
        // 尝试 2: 降级到 diagnosisService（返回原始数据）
        fullReport = await diagnosisService.getFullReport(this.data.executionId);
        console.log('[DiagnosisPage] ✅ diagnosisService 返回数据');
      }

      console.log('[DiagnosisPage] 完整报告数据结构:', {
        hasBrandDistribution: !!(fullReport?.brandDistribution?.data && Object.keys(fullReport.brandDistribution.data).length > 0),
        hasSentimentDistribution: !!(fullReport?.sentimentDistribution?.data),
        hasKeywords: !!(fullReport?.keywords && fullReport.keywords.length > 0),
        hasResults: !!(fullReport?.results && fullReport.results.length > 0),
        hasRawResults: !!(fullReport?.rawResults && fullReport.rawResults.length > 0)
      });

      // 从完整报告中提取原始数据
      const rawResults = fullReport.rawResults || 
                         fullReport.results || 
                         fullReport.detailedResults || 
                         fullReport.detailed_results ||
                         fullReport.data?.rawResults ||
                         fullReport.data?.results ||
                         [];

      // 【P0 关键修复 - 2026-03-16】从完整报告中提取品牌信息
      const brandName = fullReport.brandName || 
                        fullReport.brand_name || 
                        fullReport.mainBrand ||
                        fullReport.meta?.brandName ||
                        fullReport.execution_metadata?.main_brand ||
                        (rawResults && rawResults.length > 0 ? rawResults[0].brand : '') ||
                        '';
      
      const competitorBrands = fullReport.competitorBrands ||
                               fullReport.competitor_brands ||
                               fullReport.competitors ||
                               fullReport.meta?.competitorBrands ||
                               fullReport.execution_metadata?.competitor_brands ||
                               [];

      console.log('[DiagnosisPage] 提取的原始数据:', {
        count: rawResults.length,
        hasData: rawResults.length > 0,
        source: fullReport.brandDistribution ? 'reportService' : 'diagnosisService',
        brandName: brandName,
        competitorBrands: competitorBrands.length
      });

      // 构建看板数据
      let dashboardData = null;

      // 如果报告已经包含处理好的看板数据，直接使用
      if (fullReport.brandDistribution && fullReport.sentimentDistribution && fullReport.keywords) {
        console.log('[DiagnosisPage] ✅ 使用报告中已有的看板数据');
        dashboardData = {
          brandDistribution: fullReport.brandDistribution,
          sentimentDistribution: fullReport.sentimentDistribution,
          keywords: fullReport.keywords,
          brandScores: fullReport.brandScores || {},
          competitorAnalysis: fullReport.competitorAnalysis || {},
          semanticDriftData: fullReport.semanticDrift || fullReport.semantic_drift_data || null,
          recommendationData: fullReport.recommendations || fullReport.recommendation_data || null
        };
      } else if (rawResults && rawResults.length > 0) {
        // 否则从原始数据生成
        console.log('[DiagnosisPage] 从原始数据生成看板数据...');
      ...
      if (dashboardData) {
        console.log('[DiagnosisPage] 看板数据:', {
        ...
        console.warn('[DiagnosisPage] ⚠️ 无法生成看板数据');
        console.warn('[DiagnosisPage] 完整报告:', JSON.stringify(fullReport).substring(0, 500));
      }
    } catch (error) {
