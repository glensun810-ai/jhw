...
class PollingManager {
  ...
  constructor(options = {}) {
    ...
    this.stageIntervals = {
      init: 3000,     // init 阶段：3 秒轮询一次（优化：从 2 秒增加）
      fast: 2000,     // 0-30%: 2 秒轮询一次（优化：从 1 秒增加）
      medium: 3000,   // 30-60%: 3 秒轮询一次（优化：从 2 秒增加）
      slow: 4000,     // 60-80%: 4 秒轮询一次（优化：从 3 秒增加）
      final: 5000     // 80-100%: 5 秒轮询一次（保持不变）
    };
