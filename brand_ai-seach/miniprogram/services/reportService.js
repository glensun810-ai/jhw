...
class ReportService {
   ...
   * @param {string} executionId - 执行 ID
   * @returns {Promise<Object>} 报告数据（经过处理）
   * @private
   */
  _getFullReportViaHttp(executionId) {
    // 【P0 修复 - 2026-03-16】使用全局配置中的服务器地址，与 diagnosisService 保持一致
    const app = getApp();
    const API_BASE_URL = app.globalData.serverUrl || 'http://127.0.0.1:5001';

    var that = this;
    return new Promise(function(resolve, reject) {
      console.log('[ReportService] _getFullReportViaHttp:', executionId, 'URL:', API_BASE_URL);
      ...
      wx.request({
        ...
        success: function(res) {
          console.log('[ReportService] HTTP 响应状态码:', res.statusCode);
          console.log('[ReportService] HTTP 完整响应:', JSON.stringify({
            statusCode: res.statusCode,
            errMsg: res.errMsg,
            hasData: !!res.data,
            dataType: typeof res.data,
            dataKeys: res.data ? Object.keys(res.data) : [],
            dataString: res.data ? JSON.stringify(res.data).substring(0, 300) : 'undefined'
          }, null, 2));

          // 检查 res 是否存在
          if (!res) {
            console.error('[ReportService] ❌ HTTP 请求返回为空');
            reject(new Error('HTTP 请求返回为空，请检查网络'));
            return;
          }

          // 【P0 修复 - 2026-03-16】检查 statusCode
          if (res.statusCode !== 200) {
            console.error('[ReportService] ❌ HTTP 状态码错误:', res.statusCode);
            reject(new Error('后端返回状态码错误：' + res.statusCode));
            return;
          }
          ...
          if (!result) {
            console.error('[ReportService] ❌ HTTP 返回 data 为空，完整 res:', JSON.stringify(res));
            reject(new Error('HTTP 返回为空'));
            return;
          }

          console.log('[ReportService] HTTP 响应数据长度:', JSON.stringify(result).length);

          // 【P0 修复 - 2026-03-16】处理后端返回格式：{ success: true, data: report }
          var report = null;
          if (result.success && result.data) {
            report = result.data;
            console.log('[ReportService] 使用 result.data 作为报告数据');
          } else if (result.report) {
            report = result;
            console.log('[ReportService] 使用 result 作为报告数据（兼容格式）');
          } else {
            report = result;
            console.log('[ReportService] 直接使用 result 作为报告数据');
          }
          
          // 检查后端返回的错误
          if (report.error_code || report.error) {
            console.error('[ReportService] ❌ 后端返回错误:', report.error_code, report.error_message || report.error);
            var error = new Error(report.error_message || report.error || '获取报告失败');
            error.code = report.error_code || 'REPORT_ERROR';
            reject(error);
            return;
          }

          console.log('[ReportService] ✅ HTTP 获取成功，开始处理报告数据...');

          // 【P0 关键修复】处理报告数据（添加 brandDistribution, sentimentDistribution, keywords 等）
          var processedReport = that._processReportData(report);
        ...
        fail: function(error) {
          console.error('[ReportService] ❌ HTTP 请求失败:', error);

          // 提供更友好的错误消息
          var errorMsg = error.errMsg || '网络请求失败';
          if (errorMsg.includes('timeout')) {
            reject(new Error('请求超时，请检查后端服务是否启动 (' + API_BASE_URL + ')'));
          } else if (errorMsg.includes('fail')) {
            reject(new Error('无法连接后端服务，请确认 ' + API_BASE_URL + ' 可访问'));
          } else {
            reject(error);
          }
        }
  ...
  /**
   * 从结果数组计算品牌分布（增强版 - 2026-03-17）
   * @private
   * @param {Array} results - 结果数组
   * @param {Array} expectedBrands - 预期品牌列表（可选，用于降级）
   * @returns {Object} 品牌分布数据
   */
  _calculateBrandDistributionFromResults(results, expectedBrands) {
    const distribution = {};
    let totalCount = 0;
    ...
    const cleanBrandName = (brand) => {
      ...
      // 去除常见后缀词
      const suffixes = ['品牌', '公司', '汽车', '科技', '集团', 'Co.', 'Corp.', 'Inc.', 'Ltd.'];
      for (const suffix of suffixes) {
        if (brand.endsWith(suffix) && brand.length > suffix.length + 1) {
    ...
    if (results && Array.isArray(results)) {
      for (const result of results) {
        ...
        if (!brand && result.extracted_brand) {
          const cleaned = cleanBrandName(result.extracted_brand);
          if (cleaned) brand = cleaned;
        }
        
        // 层级 2: 使用 brand 字段
        if (!brand && result.brand) {
          const cleaned = cleanBrandName(result.brand);
          if (cleaned) brand = cleaned;
        }
        
        // 层级 3: 从 response 中提取预期品牌
        if (!brand && expectedBrands && result.response) {
          const responseText = typeof result.response === 'string' 
            ? result.response 
            : JSON.stringify(result.response);
          
          for (const expectedBrand of expectedBrands) {
            if (expectedBrand && responseText.includes(expectedBrand)) {
              const cleaned = cleanBrandName(expectedBrand);
              if (cleaned) {
                brand = cleaned;
                break;
              }
    ...
    if (!distribution || Object.keys(distribution).length === 0) {
      if (expectedBrands && expectedBrands.length > 0) {
        // 使用预期品牌创建空分布
        for (const expectedBrand of expectedBrands) {
          const cleaned = cleanBrandName(expectedBrand);
          distribution[cleaned || expectedBrand] = 0;
        }
      } else {
        // 使用 'Unknown'
  ...
  _calculateQualityScore(report) {
    ...
    // 品牌分布数据缺失扣分
    if (!brandDist.data || Object.keys(brandDist.data).length === 0) {
      score -= 30;
    }

    // 情感分布数据缺失扣分
    if (!sentimentDist.data || Object.keys(sentimentDist.data).length === 0) {
      score -= 20;
    }
  ...
  /**
   * P15 新增：检查是否有有效的品牌数据
   * @param {Object} brandDistribution - 品牌分布数据
   * @returns {boolean} 是否有品牌数据
   */
  hasBrandData(brandDistribution) {
    if (!brandDistribution) return false;
    
    const data = brandDistribution.data || brandDistribution._data || {};
    const totalCount = brandDistribution.totalCount || brandDistribution.total_count || 0;
    
    return Object.keys(data).length > 0 && totalCount > 0;
  }
}
