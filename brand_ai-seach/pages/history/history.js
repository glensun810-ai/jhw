...
Page({
  data: {
    ...
    // 分页 - 【修复 - 2026-03-20】改为固定页码分页，替代上滑加载
    currentPage: 1,
    pageSize: 20,  // 每页 20 条
    totalPages: 1,
    totalCount: 0,
  ...
  onReachBottom: function() {
    console.log('[页面] onReachBottom 触发');
  ...
  onScrollToLower: function() {
    console.log('[页面] onScrollToLower 触发');
    if (this.data.hasMore && !this.data.loading) {
      this.loadMore();
    }
  },
   ...
   * 【修复 - 2026-03-20】当本地数据少于 20 条时，强制从 API 加载以获取完整历史
   */
  async loadHistory() {
    ...
    try {
      ...
      try {
        ...
        // 【修复 - 2026-03-20】如果本地数据少于 100 条，强制从 API 加载
        // 原因：数据库中有 167 条历史记录（同步脚本恢复了 50 条旧数据）
        // 20 条 < 100 条，说明本地数据不完整
        if (localHistory.length < 100) {
          console.log(`[修复] 本地数据仅 ${localHistory.length} 条 < 100 条，强制从 API 加载完整历史`);
          forceApiLoad = true;
        }
      } catch (localError) {
      ...
      // 【P21 修复 - 2026-03-15】如果没有本地数据，强制从 API 加载
      // 【修复 - 2026-03-20】如果强制 API 加载，也执行此分支
      if (historyList.length === 0 || forceApiLoad) {
        console.log('[历史记录] 本地数据为空，开始从 API 加载...');
        try {
          // 【修复 - 2026-03-20】分页加载，每页 20 条
          const params = {
            page: currentPage,
            limit: this.data.pageSize,  // 每页 20 条
          ...
          console.log('[历史记录] 样本数据:', historyList.slice(0, 2).map(h => ({
            brandName: h.brandName,
            status: h.status,
            overallScore: h.overallScore
          })));

          // 【修复 - 2026-03-20】更新本地存储，保存完整历史记录
          if (historyList.length > 0) {
            try {
              // 保存到本地存储（使用 storage-manager）
              const { saveDiagnosisHistory } = require('../../utils/storage-manager');
              
              // 转换为 storage-manager 需要的格式
              const historyData = historyList.map(report => ({
                id: report.id || report.reportId,
                executionId: report.execution_id || report.executionId,
                brandName: report.brand_name || report.brandName,
                status: report.status || 'completed',
              ...
              saveDiagnosisHistory(historyData);
              console.log(`[修复] 已更新本地存储，共 ${historyData.length} 条记录`);
            } catch (storageError) {
              console.warn('[修复] 更新本地存储失败:', storageError);
            }
          }
        } catch (apiError) {
      ...
      const processedReports = historyList.map(report => {
        ...
        // 【调试 - 2026-03-20】打印品牌名，确认数据正确
        if (brandName.includes('元若曦')) {
          console.log('[历史记录] 找到元若曦记录:', {
            executionId,
            brandName,
        ...
        return {
          ...
          execution_id: executionId,  // 【P0 修复】同时保留两种格式
          // 使用统一兜底函数处理品牌名
          brandName: this.getBrandName(report),
          brand_name: this.getBrandName(report),  // 【P0 修复】同时保留两种格式
          createdAt: report.created_at || report.createdAt,
          overallScore: report.overall_score || report.overallScore || 0,
          status: report.status || 'completed',
          // 计算得分等级
          scoreLevel: this.calculateScoreLevel(report.overallScore || report.overallScore || 0),
          // 格式化时间
      ...
      // 更新数据 - 【修复 - 2026-03-20】分页模式
      // 【修复 - 2026-03-20】每页独立显示，不追加数据
      const historyListData = processedReports;

      // 计算总页数和是否有下一页
      const totalPages = Math.ceil((totalCount || historyListData.length) / this.data.pageSize);
      ...
      this.setData({
        ...
        hasMore: hasMore,  // 是否还有下一页
        isEmpty: historyListData.length === 0,
        emptyType: historyListData.length === 0 ? 'no_data' : 'no_search_result',
        loading: false
      });

      console.log(`✅ 加载历史记录成功：第 ${currentPage}/${totalPages} 页，${historyListData.length} 条，hasMore=${hasMore}, totalCount=${totalCount}`);
  ...
  /**
   * 加载更多 - 【修复 - 2026-03-20】改为加载下一页
   */
  async loadMore() {
    console.log('[加载更多] 触发加载更多，currentPage:', this.data.currentPage);
    
    if (this.data.loading || !this.data.hasMore) {
      console.log('[加载更多] 跳过：loading=', this.data.loading, 'hasMore=', this.data.hasMore);
      return;
    }

    const nextPage = this.data.currentPage + 1;
    console.log('[加载更多] 加载第', nextPage, '页');
  ...
  applyFilters() {
    ...
    // 关键词搜索 - 【修复 - 2026-03-20】增加搜索字段
    if (searchKeyword) {
      const keyword = searchKeyword.toLowerCase();
      console.log('[applyFilters] 搜索关键词:', keyword);
      
      filtered = filtered.filter(item => {
        // 搜索品牌名（支持多个字段）
        const brandName = (item.brandName || item.brand_name || '').toLowerCase();
        const executionId = (item.executionId || item.execution_id || '').toLowerCase();
        const shortId = executionId.slice(-6); // 支持搜索后 6 位 ID
        
        const match = brandName.includes(keyword) ||
               executionId.includes(keyword) ||
               shortId.includes(keyword);
        
        // 【调试 - 2026-03-20】打印匹配结果
        if (match) {
          console.log('[applyFilters] 匹配记录:', {
            brandName,
            executionId,
            shortId
          });
      ...
      console.log('[applyFilters] 搜索匹配数:', filtered.length);
    }

    // 品牌筛选
    if (filterBrand) {
      filtered = filtered.filter(item =>
        (item.brandName || item.brand_name || '').toLowerCase().includes(filterBrand.toLowerCase())
      );
    }
  ...
  onSelectAll() {
    ...
    if (selectAll) {
      ...
      this.setData({
        selectedIds: filteredList.map(item => item.executionId),
        selectAll: true
      });
    }
  },
        ...
        if (res.confirm) {
          ...
          try {
            ...
            const executionIds = filteredList
              .filter(item => selectedIds.includes(item.executionId))
            ...
            // 尝试调用批量删除 API（如果后端支持）
            try {
              await batchDeleteHistoryReports(executionIds);
            } catch (apiError) {
              console.warn('[批量删除] API 删除失败，仅删除本地存储:', apiError);
            }

            // 从列表中移除
            const newHistoryList = historyList.filter(item => !selectedIds.includes(item.executionId));
            const newFilteredList = filteredList.filter(item => !selectedIds.includes(item.executionId));
