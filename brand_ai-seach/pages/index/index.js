...
Page({
  ...
  data: {
    ...
    // 问题设置
    customQuestions: [{text: '请介绍{{brandName}}品牌的核心产品与服务', show: true}, {text: '{{brandName}}的主要竞争优势是什么', show: true}, {text: '用户对{{brandName}}的评价如何', show: true}],
    ...
    config: {
      ...
      brandName: '',
      competitorBrands: [],
      // 【P0 修复 - 2026-03-20】默认 3 个标准问题
      customQuestions: [
        {text: '请介绍{{brandName}}品牌的核心产品与服务', show: true},
        {text: '{{brandName}}的主要竞争优势是什么', show: true},
        {text: '用户对{{brandName}}的评价如何', show: true}
      ]
    },
  ...
  onShow: function() {
    // 【P0 关键修复 - 2026-03-15】先确保 AI 平台数据已初始化，再进行其他操作
    // 修复根因：之前 applyConfig 在 refreshAiPlatforms 之前调用，导致 AI 平台数据可能为空
    this.refreshAiPlatforms();

    // 检查是否有从配置管理页面传回的临时配置
    const app = getApp();
    if (app.globalData && app.globalData.tempConfig) {
      // 【关键修复】此时 AI 平台数据已确保初始化，可以安全应用配置
  ...
  /**
   * P2 修复：应用配置（增强数据防御性）
   * 【关键修复 - 2026-03-15】确保 AI 平台矩阵数据在映射前已初始化，杜绝消失问题
   */
  applyConfig: function(config) {
    ...
    // 【关键修复】确保 AI 平台数据已初始化，再进行配置应用
    // 如果 domesticAiModels 或 overseasAiModels 为空，先初始化
    if (!this.data.domesticAiModels || !Array.isArray(this.data.domesticAiModels) || this.data.domesticAiModels.length === 0) {
      console.log('[applyConfig] domesticAiModels 为空，先初始化');
      this.initDomesticAiModels();
    }
    if (!this.data.overseasAiModels || !Array.isArray(this.data.overseasAiModels) || this.data.overseasAiModels.length === 0) {
      console.log('[applyConfig] overseasAiModels 为空，先初始化');
      this.initOverseasAiModels();
    }
    ...
    // 【关键修复】使用完整的默认模型列表作为基础，确保不会丢失任何平台
    // 【P0 修复 - 2026-03-20】默认勾选 4 个主流模型，确保诊断结果数量充足
    const defaultDomesticModels = [
      { name: 'DeepSeek', id: 'deepseek', checked: true, logo: 'DS', tags: ['综合', '代码'] },  // ✅ 默认勾选
      { name: '豆包', id: 'doubao', checked: true, logo: 'DB', tags: ['综合', '创意'] },  // ✅ 默认勾选
      { name: '通义千问', id: 'qwen', checked: true, logo: 'QW', tags: ['综合', '长文本'] },  // ✅ 默认勾选
      { name: '元宝', id: 'yuanbao', checked: false, logo: 'YB', tags: ['综合']},
      { name: 'Kimi', id: 'kimi', checked: false, logo: 'KM', tags: ['长文本'] },
      { name: '文心一言', id: 'wenxin', checked: false, logo: 'WX', tags: ['综合', '创意'] },
      { name: '讯飞星火', id: 'xinghuo', checked: false, logo: 'XF', tags: ['综合', '语音'] },
      { name: '智谱 AI', id: 'zhipu', checked: true, logo: 'ZP', tags: ['综合', 'GLM'] },  // ✅ 默认勾选
    ];

    const defaultOverseasModels = [
      { name: 'ChatGPT', id: 'chatgpt', checked: false, logo: 'GPT', tags: ['综合', '代码'] },
      { name: 'Gemini', id: 'gemini', checked: false, logo: 'GM', tags: ['综合', '多模态'] },
      { name: 'Claude', id: 'claude', checked: false, logo: 'CD', tags: ['长文本', '创意'] },
      { name: 'Perplexity', id: 'perplexity', checked: false, logo: 'PE', tags: ['综合', '长文本'] },
      { name: 'Grok', id: 'grok', checked: false, logo: 'GR', tags: ['推理', '多模态'] },
    ...
    // 【关键修复】基于默认列表恢复选中状态，而不是基于可能为空的当前数据
    const restoredDomesticModels = defaultDomesticModels.map(defaultModel => {
      // 优先使用配置中保存的选中状态
      const savedModel = domesticAiModelsConfig.find(saved => saved && saved.name === defaultModel.name);
      if (savedModel) {
        return {
          ...defaultModel,
          checked: savedModel.checked !== undefined ? savedModel.checked : false
        };
      }
      // 如果配置中没有，使用默认模型的选中状态
      return { ...defaultModel };
    ...
    const restoredOverseasModels = defaultOverseasModels.map(defaultModel => {
      // 优先使用配置中保存的选中状态
      const savedModel = overseasAiModelsConfig.find(saved => saved && saved.name === defaultModel.name);
      if (savedModel) {
        return {
          ...defaultModel,
          checked: savedModel.checked !== undefined ? savedModel.checked : false
    ...
    this.setData({
      ...
      domesticAiModels: restoredDomesticModels,
      overseasAiModels: restoredOverseasModels
    });

    // 更新选中计数
    this.updateSelectedModelCount();

    console.log('[applyConfig] 配置已应用', {
      domestic: restoredDomesticModels.length,
      overseas: restoredOverseasModels.length,
      domesticSelected: restoredDomesticModels.filter(m => m.checked).length,
      overseasSelected: restoredOverseasModels.filter(m => m.checked).length
    });
  ...
  initDomesticAiModels: function() {
    const defaultDomestic = [
      ...
      { name: '文心一言', id: 'wenxin', checked: false, logo: 'WX', tags: ['综合', '创意'] },
      { name: '讯飞星火', id: 'xinghuo', checked: false, logo: 'XF', tags: ['综合', '语音'] },
      { name: '智谱 AI', id: 'zhipu', checked: true, logo: 'ZP', tags: ['综合', 'GLM'] },
    ];
  ...
  resetAllInput: function() {
    ...
    this.setData({ customQuestions: [{text: '请介绍{{brandName}}品牌的核心产品与服务', show: true}, {text: '{{brandName}}的主要竞争优势是什么', show: true}, {text: '用户对{{brandName}}的评价如何', show: true}] });
  ...
  handleDiagnosisComplete: function(parsedStatus, executionId) {
    try {
      ...
      const saveSuccess = saveDiagnosisResult(executionId, {
        ...
        // 【首席工程师修复 - 2026-03-15】不再保存 rawResponse
        // rawResponse 包含完整的 API 响应，数据量过大（可能几百 KB 甚至几 MB）
        // 这是导致历史诊断记录详情页卡死的根本原因
        // 详情页只需要结构化的诊断数据，不需要完整的原始响应
