...
const { getFullReportApi } = require('../api/home');
...
/**
 * 【P0 关键修复 - 2026-03-16】获取完整诊断报告
 * WebSocket 完成消息不包含详细结果，需要主动调用 API 获取
 * @param {string} executionId - 执行 ID
 * @returns {Promise<Object>} 完整报告数据
 */
const getFullReport = (executionId) => {
  console.log('[diagnosisService] Getting full report for:', executionId);
  return getFullReportApi(executionId)
    .then(res => {
      console.log('[diagnosisService] Full report received:', executionId);
      return res;
    })
    .catch(error => {
      console.error('[diagnosisService] Failed to get full report:', executionId, error);
      throw error;
    });
};
